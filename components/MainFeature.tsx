...
const DecoratorBlob = styled("img", { base: `w-20 h-20 absolute right-0 bottom-0 transform translate-x-1/2 translate-y-1/2 fill-current text-primary-500 -z-10` })
...
const Heading = styled(
  SectionHeading, { base: `mt-4 font-black text-left text-3xl sm:text-4xl lg:text-5xl text-center md:text-left leading-tight` }
);
const Description = styled("p", { base: `mt-4 text-center md:text-left text-sm md:text-base lg:text-lg font-medium leading-relaxed text-secondary-100` });
...
const MainFeature: React.FC<MainFeatureProps> = ({
  ...
  heading = (
    <>
      Designed & Developed by <span className={tw`text-primary-500`}>Professionals.</span>
    </>
  ),
  ...
  return (
    <Container>
      <TwoColumn>
        <ImageColumn>
          <Image className={imageClass} src={imageSrc} imageBorder={imageBorder} imageShadow={imageShadow} imageRounded={imageRounded} />
          {/* FIXME: image does not work */}
          {imageDecoratorBlob && <DecoratorBlob src={SvgDotPattern} alt="DecoratorBlob" className={imageDecoratorBlobClass} />}
        </ImageColumn>
